...
**Large Chinese font files**: `TsangerJinKai02-W04.ttf` and `TsangerJinKai02-W05.ttf` are close to 19MB each. This Codex skill keeps them under `assets/fonts` so local WeasyPrint renders can use the template-relative `@font-face` paths. If a copied or trimmed skill does not include them, use `scripts/ensure-fonts.sh`.

When Chinese fonts are missing, `scripts/ensure-fonts.sh` downloads them to the XDG user font dir (`${XDG_DATA_HOME:-~/.local/share}/fonts/kami`, override with `KAMI_FONT_DIR`), **not** into the skill's `assets/fonts`. fontconfig scans that dir by default on macOS and Linux, so WeasyPrint resolves `TsangerJinKai02` from there; online renders still use the jsDelivr `@font-face` URL.
...
**Auto-inference rules** (Codex fills these from the document content without asking):
...
Use the `npx @marp-team/marp-cli@latest ...` form below for zero-install. For repeat use, install via `npm i -g @marp-team/marp-cli` or `brew install marp-cli` (see [marp-cli docs](https://github.com/marp-team/marp-cli)). Kami's build and stabilization helpers do not call `marp`.
...
If the output shows `DejaVuSerif` / `Bitstream Vera` - your specified font didn't load, fell through to system ultimate fallback. Expected: `Charter`, `Georgia`, `TsangerJinKai02`, `Source-Han-Serif-SC`, or another configured Chinese/English fallback.
...
### 20. (P0) Template HTML must reference assets inside the skill directory

**Symptom**: Image slot renders as a missing-image placeholder in the PDF; rendered output looks empty where a screenshot should be.

**Root cause**: An `<img src="../../../sibling-project/asset.jpg">` reaches outside the skill directory. The path resolves on the maintainer's laptop where the sibling project happens to be checked out, but breaks for other users and any environment that does not recreate that working tree.

**Fix**: Every reusable image referenced by a template must live under `assets/illustrations/`, `assets/images/`, or beside the generated output file. Copy external sources into the working project, then reference them with a relative path inside that project or skill directory.
...
<!-- use -->
<img src="../illustrations/travel-spatialvla.png" alt="...">
```

Quick check before rendering a filled document: inspect image `src` values and avoid `/Users/...`, `file://`, or paths that depend on an unrelated sibling checkout.
