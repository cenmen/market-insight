const etf588200 = {
  etf: {
    code: '588200',
    name: '科创芯片ETF',
    index: '上证科创板芯片指数',
    intro: '跟踪上证科创板芯片指数，组合集中在晶圆制造、国产AI芯片、半导体设备、存储和高速光芯片等科创板硬科技方向，前十大权重对净值弹性影响较高。',
    concepts: ['科创芯片', '国产半导体', 'AI算力芯片', '晶圆制造', '半导体设备', '存储芯片', 'Chiplet', '高速光芯片'],
    scale: '386.07 亿元',
  },
  report: {
    date: '2026-06-02',
    headlineSignal: '规模与流动性观察',
    coreJudgment: '国产芯片主线的高弹性组合',
    thesis:
      '跟踪上证科创板芯片指数，组合暴露集中在晶圆代工、AI算力芯片、内存接口、半导体设备和光芯片等方向。它不是低波动红利资产，而是偏国产半导体景气、AI算力需求和科创板风险偏好的高弹性ETF。',
    callout: '适合用来跟踪国产芯片和AI算力芯片主线的风险偏好，但短线涨跌受龙头股交易拥挤、设备订单验证和科创板情绪影响很大。',
    chartCaption: '近期走势显示芯片主线弹性明显放大，连续上冲后短线回撤和宽幅震荡也同步增加。',
    hiddenStoryLine: '资金还在围绕国产替代和 AI 算力芯片做高位轮动，设备和存储的回流，说明这条线不是单点冲刺，而是在等更大的产业链扩散。',
    timelineEvents: [
      {
        datetime: '2026-05-20',
        title: '芯片股继续拉升',
        summary: '主线权重股延续强势上攻，说明市场仍在给国产替代和AI 算力芯片定价，但涨速过快也在积累短线分歧。',
        source: '近两周盘面模拟',
        statusTag: { label: '偏强', kind: 'bullish' },
        tags: ['趋势向上', '情绪高位', '交易拥挤'],
      },
      {
        datetime: '2026-05-26',
        title: '设备链承接增强',
        summary: '半导体设备和存储方向出现回流，说明资金开始从单一龙头向产业链扩散，板块内部承接比前几天更稳。',
        source: '近两周盘面模拟',
        statusTag: { label: '扩散', kind: 'neutral' },
        tags: ['资金扩散', '板块轮动', '承接改善'],
      },
      {
        datetime: '2026-06-01',
        title: '高位回撤确认',
        summary: '指数在前高附近受阻后回落，说明短线追高资金开始兑现，后续更看回踩后的承接而不是单纯冲高。',
        source: '近两周盘面模拟',
        statusTag: { label: '分歧', kind: 'neutral' },
        tags: ['前高受阻', '回踩确认', '节奏放慢'],
      },
    ],
    risks: [
      {
        title: '高波动风险',
        description: '科创板芯片权重弹性较高，短线情绪降温时净值回撤可能明显放大。',
      },
      {
        title: '业绩兑现风险',
        description: 'AI芯片、设备和存储公司的高增速需要订单、交付和毛利率持续验证。',
      },
      {
        title: '估值拥挤风险',
        description: '核心权重若涨幅过快，容易在放量滞涨或财报低于预期时出现估值压缩。',
      },
      {
        title: '产业周期风险',
        description: '晶圆厂资本开支、存储价格周期和半导体设备招标节奏都会影响组合景气度。',
      },
    ],
  },
  metrics: [
    {
      label: '基金规模',
      value: '386.07 亿',
      note: '资金流动性',
    },
    {
      label: '盈利增速',
      value: '227.55%',
      note: '持仓盈利增速估算',
    },
    {
      label: 'ROE',
      value: '4.61%',
      note: '持仓加权 ROE 估算',
    },
    {
      label: '业务增速',
      value: '93.91%',
      note: '持仓业务增速估算',
    },
  ],
  recentFiveDayAmplitude: '5.74%（4.26% ~ 7.61%）',
  recentFiveDayAmplitudeComment: '近 5 日振幅中枢偏高，说明资金分歧和价格弹性都在放大，短线更容易出现宽幅震荡。',
  recentTenDayMaxDrawdown: '-6.98%',
  recentTenDayMaxDrawdownDate: '2026.05.29',
  recentTenDayMaxDrawdownComment: '10 日内最大回撤接近 7%，回撤压力较明显，观察重点是回踩后能否重新收回短期均线。',
  businessRatio: [
    {
      type: 'AI算力芯片与IP',
      rate: 23.27,
      desc: '海光信息、寒武纪、芯原股份构成国产AI算力芯片、处理器和芯片设计服务暴露。',
      subItems: [
        {
          type: '海光信息',
          rate: 9.66,
        },
        {
          type: '寒武纪',
          rate: 9.56,
        },
        {
          type: '芯原股份',
          rate: 4.05,
        },
      ],
    },
    {
      type: '晶圆制造与特色工艺',
      rate: 12.15,
      desc: '中芯国际、华虹公司代表先进制程、成熟制程和特色工艺晶圆代工能力。',
      subItems: [
        {
          type: '中芯国际',
          rate: 9.8,
        },
        {
          type: '华虹公司',
          rate: 2.35,
        },
      ],
    },
    {
      type: '存储与接口芯片',
      rate: 11.57,
      desc: '澜起科技、佰维存储分别对应服务器内存接口芯片和存储模组周期修复。',
      subItems: [
        {
          type: '澜起科技',
          rate: 7.8,
        },
        {
          type: '佰维存储',
          rate: 3.77,
        },
      ],
    },
    {
      type: '半导体设备',
      rate: 10.69,
      desc: '中微公司、拓荆科技覆盖刻蚀、薄膜沉积等国产半导体设备关键环节。',
      subItems: [
        {
          type: '中微公司',
          rate: 7.3,
        },
        {
          type: '拓荆科技',
          rate: 3.39,
        },
      ],
    },
    {
      type: '光芯片与高速互联',
      rate: 3.29,
      desc: '源杰科技提供高速光芯片和激光器芯片暴露，受益AI数据中心互联升级。',
      subItems: [
        {
          type: '源杰科技',
          rate: 3.29,
        },
      ],
    },
    {
      type: '其他科创芯片成分',
      rate: 39.03,
      desc: '剩余成分分布在半导体设计、制造、材料、设备和电子核心器件等环节。',
    },
  ],
  shortTermFactors: [
    {
      title: 'AI算力芯片订单',
      description: '海光信息、寒武纪等权重的订单能见度和交付节奏决定组合进攻弹性。',
    },
    {
      title: '晶圆厂资本开支',
      description: '中芯国际、华虹公司扩产和设备采购节奏会影响制造与设备链情绪。',
    },
    {
      title: '半导体设备验证',
      description: '刻蚀、薄膜沉积等国产设备的客户验证和招标进展影响业绩兑现预期。',
    },
    {
      title: '存储周期与服务器升级',
      description: '存储价格、DDR5渗透和AI服务器平台升级会影响澜起科技、佰维存储等方向。',
    },
    {
      title: '科创板交易拥挤度',
      description: '核心权重涨幅和成交占比偏高时，放量滞涨容易放大ETF短线波动。',
    },
  ],
  styleCharacteristics: [
    {
      title: '国产芯片高弹性',
      description: '组合集中在科创板半导体龙头，受国产替代、AI算力和产业政策共振影响。',
    },
    {
      title: '龙头权重集中',
      description: '中芯国际、海光信息、寒武纪等权重较高，单股波动会显著影响净值。',
    },
    {
      title: '景气验证驱动',
      description: '行情持续性依赖财报增速、订单披露、设备招标和晶圆厂扩产验证。',
    },
    {
      title: '短线波动偏大',
      description: '近期K线振幅抬升，适合关注回撤后的风险收益比，而不是追逐单日情绪。',
    },
  ],
  story: null,
  tPrinciples: null,
  tReferences: null,
  strategies: null,
  kLineData: [
    {
      date: '2026-03-05',
      open: 2.583,
      close: 2.562,
      low: 2.541,
      high: 2.604,
      volume: 1124949820,
      amount: 2897287200,
      amplitude: 0,
      changePercent: 0,
      changeAmount: 0,
      turnoverRate: 0,
    },
    {
      date: '2026-03-06',
      open: 2.538,
      close: 2.565,
      low: 2.537,
      high: 2.59,
      volume: 693283990,
      amount: 1780371800,
      amplitude: 2.07,
      changePercent: 0.12,
      changeAmount: 0.003,
      turnoverRate: 0,
    },
    {
      date: '2026-03-09',
      open: 2.5,
      close: 2.513,
      low: 2.429,
      high: 2.528,
      volume: 1173742360,
      amount: 2895764000,
      amplitude: 3.86,
      changePercent: -2.03,
      changeAmount: -0.052,
      turnoverRate: 0,
    },
    {
      date: '2026-03-10',
      open: 2.575,
      close: 2.579,
      low: 2.545,
      high: 2.594,
      volume: 889236790,
      amount: 2289260300,
      amplitude: 1.95,
      changePercent: 2.63,
      changeAmount: 0.066,
      turnoverRate: 0,
    },
    {
      date: '2026-03-11',
      open: 2.58,
      close: 2.541,
      low: 2.538,
      high: 2.603,
      volume: 664407680,
      amount: 1704989800,
      amplitude: 2.52,
      changePercent: -1.47,
      changeAmount: -0.038,
      turnoverRate: 0,
    },
    {
      date: '2026-03-12',
      open: 2.537,
      close: 2.506,
      low: 2.482,
      high: 2.574,
      volume: 746675060,
      amount: 1883455400,
      amplitude: 3.62,
      changePercent: -1.38,
      changeAmount: -0.035,
      turnoverRate: 0,
    },
    {
      date: '2026-03-13',
      open: 2.483,
      close: 2.476,
      low: 2.456,
      high: 2.514,
      volume: 703491020,
      amount: 1744139700,
      amplitude: 2.31,
      changePercent: -1.2,
      changeAmount: -0.03,
      turnoverRate: 0,
    },
    {
      date: '2026-03-16',
      open: 2.461,
      close: 2.521,
      low: 2.425,
      high: 2.525,
      volume: 1059502150,
      amount: 2618066900,
      amplitude: 4.04,
      changePercent: 1.82,
      changeAmount: 0.045,
      turnoverRate: 0,
    },
    {
      date: '2026-03-17',
      open: 2.519,
      close: 2.447,
      low: 2.443,
      high: 2.521,
      volume: 774521740,
      amount: 1921634800,
      amplitude: 3.09,
      changePercent: -2.94,
      changeAmount: -0.074,
      turnoverRate: 0,
    },
    {
      date: '2026-03-18',
      open: 2.46,
      close: 2.496,
      low: 2.443,
      high: 2.5,
      volume: 1034483540,
      amount: 2559542300,
      amplitude: 2.33,
      changePercent: 2,
      changeAmount: 0.049,
      turnoverRate: 0,
    },
    {
      date: '2026-03-19',
      open: 2.445,
      close: 2.436,
      low: 2.422,
      high: 2.464,
      volume: 1056071740,
      amount: 2581717000,
      amplitude: 1.68,
      changePercent: -2.4,
      changeAmount: -0.06,
      turnoverRate: 0,
    },
    {
      date: '2026-03-20',
      open: 2.449,
      close: 2.409,
      low: 2.406,
      high: 2.467,
      volume: 1134793410,
      amount: 2771972100,
      amplitude: 2.5,
      changePercent: -1.11,
      changeAmount: -0.027,
      turnoverRate: 0,
    },
    {
      date: '2026-03-23',
      open: 2.36,
      close: 2.292,
      low: 2.277,
      high: 2.378,
      volume: 1330832660,
      amount: 3098932100,
      amplitude: 4.19,
      changePercent: -4.86,
      changeAmount: -0.117,
      turnoverRate: 0,
    },
    {
      date: '2026-03-24',
      open: 2.32,
      close: 2.35,
      low: 2.262,
      high: 2.352,
      volume: 1185809620,
      amount: 2735078200,
      amplitude: 3.93,
      changePercent: 2.53,
      changeAmount: 0.058,
      turnoverRate: 0,
    },
    {
      date: '2026-03-25',
      open: 2.373,
      close: 2.413,
      low: 2.363,
      high: 2.434,
      volume: 1177103400,
      amount: 2835767500,
      amplitude: 3.02,
      changePercent: 2.68,
      changeAmount: 0.063,
      turnoverRate: 0,
    },
    {
      date: '2026-03-26',
      open: 2.397,
      close: 2.363,
      low: 2.355,
      high: 2.41,
      volume: 798518330,
      amount: 1900409900,
      amplitude: 2.28,
      changePercent: -2.07,
      changeAmount: -0.05,
      turnoverRate: 0,
    },
    {
      date: '2026-03-27',
      open: 2.33,
      close: 2.383,
      low: 2.311,
      high: 2.405,
      volume: 774041410,
      amount: 1827973800,
      amplitude: 3.98,
      changePercent: 0.85,
      changeAmount: 0.02,
      turnoverRate: 0,
    },
    {
      date: '2026-03-30',
      open: 2.33,
      close: 2.372,
      low: 2.322,
      high: 2.381,
      volume: 778068670,
      amount: 1829797500,
      amplitude: 2.48,
      changePercent: -0.46,
      changeAmount: -0.011,
      turnoverRate: 0,
    },
    {
      date: '2026-03-31',
      open: 2.353,
      close: 2.294,
      low: 2.29,
      high: 2.368,
      volume: 748692610,
      amount: 1742207600,
      amplitude: 3.29,
      changePercent: -3.29,
      changeAmount: -0.078,
      turnoverRate: 0,
    },
    {
      date: '2026-04-01',
      open: 2.358,
      close: 2.377,
      low: 2.341,
      high: 2.384,
      volume: 1130433210,
      amount: 2672481700,
      amplitude: 1.87,
      changePercent: 3.62,
      changeAmount: 0.083,
      turnoverRate: 0,
    },
    {
      date: '2026-04-02',
      open: 2.369,
      close: 2.307,
      low: 2.287,
      high: 2.371,
      volume: 870489350,
      amount: 2023995600,
      amplitude: 3.53,
      changePercent: -2.94,
      changeAmount: -0.07,
      turnoverRate: 0,
    },
    {
      date: '2026-04-03',
      open: 2.328,
      close: 2.311,
      low: 2.302,
      high: 2.34,
      volume: 684335370,
      amount: 1589187000,
      amplitude: 1.65,
      changePercent: 0.17,
      changeAmount: 0.004,
      turnoverRate: 0,
    },
    {
      date: '2026-04-07',
      open: 2.317,
      close: 2.36,
      low: 2.315,
      high: 2.383,
      volume: 912959850,
      amount: 2154091800,
      amplitude: 2.94,
      changePercent: 2.12,
      changeAmount: 0.049,
      turnoverRate: 0,
    },
    {
      date: '2026-04-08',
      open: 2.451,
      close: 2.514,
      low: 2.438,
      high: 2.515,
      volume: 1352081300,
      amount: 3346592200,
      amplitude: 3.26,
      changePercent: 6.53,
      changeAmount: 0.154,
      turnoverRate: 0,
    },
    {
      date: '2026-04-09',
      open: 2.475,
      close: 2.519,
      low: 2.475,
      high: 2.557,
      volume: 1071099950,
      amount: 2702944500,
      amplitude: 3.26,
      changePercent: 0.2,
      changeAmount: 0.005,
      turnoverRate: 0,
    },
    {
      date: '2026-04-10',
      open: 2.557,
      close: 2.565,
      low: 2.557,
      high: 2.596,
      volume: 946250330,
      amount: 2435992600,
      amplitude: 1.55,
      changePercent: 1.83,
      changeAmount: 0.046,
      turnoverRate: 0,
    },
    {
      date: '2026-04-13',
      open: 2.544,
      close: 2.589,
      low: 2.544,
      high: 2.641,
      volume: 1089021630,
      amount: 2830978700,
      amplitude: 3.78,
      changePercent: 0.94,
      changeAmount: 0.024,
      turnoverRate: 0,
    },
    {
      date: '2026-04-14',
      open: 2.629,
      close: 2.65,
      low: 2.615,
      high: 2.669,
      volume: 1157811420,
      amount: 3055069500,
      amplitude: 2.09,
      changePercent: 2.36,
      changeAmount: 0.061,
      turnoverRate: 0,
    },
    {
      date: '2026-04-15',
      open: 2.67,
      close: 2.648,
      low: 2.631,
      high: 2.715,
      volume: 1198752950,
      amount: 3207966200,
      amplitude: 3.17,
      changePercent: -0.08,
      changeAmount: -0.002,
      turnoverRate: 0,
    },
    {
      date: '2026-04-16',
      open: 2.651,
      close: 2.679,
      low: 2.63,
      high: 2.68,
      volume: 843112250,
      amount: 2241775900,
      amplitude: 1.89,
      changePercent: 1.17,
      changeAmount: 0.031,
      turnoverRate: 0,
    },
    {
      date: '2026-04-17',
      open: 2.666,
      close: 2.699,
      low: 2.665,
      high: 2.723,
      volume: 874958830,
      amount: 2361670200,
      amplitude: 2.16,
      changePercent: 0.75,
      changeAmount: 0.02,
      turnoverRate: 0,
    },
    {
      date: '2026-04-20',
      open: 2.701,
      close: 2.745,
      low: 2.701,
      high: 2.747,
      volume: 971035690,
      amount: 2650918900,
      amplitude: 1.7,
      changePercent: 1.7,
      changeAmount: 0.046,
      turnoverRate: 0,
    },
    {
      date: '2026-04-21',
      open: 2.733,
      close: 2.71,
      low: 2.665,
      high: 2.734,
      volume: 634350150,
      amount: 1710790300,
      amplitude: 2.51,
      changePercent: -1.28,
      changeAmount: -0.035,
      turnoverRate: 0,
    },
    {
      date: '2026-04-22',
      open: 2.693,
      close: 2.778,
      low: 2.687,
      high: 2.779,
      volume: 974751240,
      amount: 2676555000,
      amplitude: 3.39,
      changePercent: 2.51,
      changeAmount: 0.068,
      turnoverRate: 0,
    },
    {
      date: '2026-04-23',
      open: 2.805,
      close: 2.735,
      low: 2.702,
      high: 2.81,
      volume: 1068405440,
      amount: 2943764500,
      amplitude: 3.89,
      changePercent: -1.55,
      changeAmount: -0.043,
      turnoverRate: 0,
    },
    {
      date: '2026-04-24',
      open: 2.746,
      close: 2.806,
      low: 2.717,
      high: 2.844,
      volume: 1661477500,
      amount: 4634837000,
      amplitude: 4.64,
      changePercent: 2.6,
      changeAmount: 0.071,
      turnoverRate: 0,
    },
    {
      date: '2026-04-27',
      open: 2.856,
      close: 2.938,
      low: 2.832,
      high: 2.947,
      volume: 1380572600,
      amount: 4004155500,
      amplitude: 4.1,
      changePercent: 4.7,
      changeAmount: 0.132,
      turnoverRate: 0,
    },
    {
      date: '2026-04-28',
      open: 2.916,
      close: 2.893,
      low: 2.872,
      high: 2.972,
      volume: 1225546370,
      amount: 3577899900,
      amplitude: 3.4,
      changePercent: -1.53,
      changeAmount: -0.045,
      turnoverRate: 0,
    },
    {
      date: '2026-04-29',
      open: 2.853,
      close: 2.899,
      low: 2.798,
      high: 2.907,
      volume: 1157522690,
      amount: 3319091500,
      amplitude: 3.77,
      changePercent: 0.21,
      changeAmount: 0.006,
      turnoverRate: 0,
    },
    {
      date: '2026-04-30',
      open: 2.94,
      close: 3.088,
      low: 2.938,
      high: 3.11,
      volume: 1564180700,
      amount: 4733439400,
      amplitude: 5.93,
      changePercent: 6.52,
      changeAmount: 0.189,
      turnoverRate: 0,
    },
    {
      date: '2026-05-06',
      open: 3.211,
      close: 3.26,
      low: 3.211,
      high: 3.399,
      volume: 1595068600,
      amount: 5273270700,
      amplitude: 6.09,
      changePercent: 5.57,
      changeAmount: 0.172,
      turnoverRate: 0,
    },
    {
      date: '2026-05-07',
      open: 3.262,
      close: 3.332,
      low: 3.233,
      high: 3.342,
      volume: 1108514390,
      amount: 3647080900,
      amplitude: 3.34,
      changePercent: 2.21,
      changeAmount: 0.072,
      turnoverRate: 0,
    },
    {
      date: '2026-05-08',
      open: 3.267,
      close: 3.241,
      low: 3.203,
      high: 3.273,
      volume: 1379031100,
      amount: 4467169100,
      amplitude: 2.1,
      changePercent: -2.73,
      changeAmount: -0.091,
      turnoverRate: 0,
    },
    {
      date: '2026-05-11',
      open: 3.349,
      close: 3.428,
      low: 3.306,
      high: 3.455,
      volume: 1649679800,
      amount: 5590896900,
      amplitude: 4.6,
      changePercent: 5.77,
      changeAmount: 0.187,
      turnoverRate: 0,
    },
    {
      date: '2026-05-12',
      open: 3.411,
      close: 3.467,
      low: 3.37,
      high: 3.519,
      volume: 1516247800,
      amount: 5222596000,
      amplitude: 4.35,
      changePercent: 1.14,
      changeAmount: 0.039,
      turnoverRate: 0,
    },
    {
      date: '2026-05-13',
      open: 3.374,
      close: 3.566,
      low: 3.351,
      high: 3.569,
      volume: 1413906500,
      amount: 4891274200,
      amplitude: 6.29,
      changePercent: 2.86,
      changeAmount: 0.099,
      turnoverRate: 0,
    },
    {
      date: '2026-05-14',
      open: 3.611,
      close: 3.483,
      low: 3.475,
      high: 3.634,
      volume: 1592235900,
      amount: 5642935000,
      amplitude: 4.46,
      changePercent: -2.33,
      changeAmount: -0.083,
      turnoverRate: 0,
    },
    {
      date: '2026-05-15',
      open: 3.472,
      close: 3.422,
      low: 3.354,
      high: 3.594,
      volume: 2176340100,
      amount: 7518612100,
      amplitude: 6.89,
      changePercent: -1.75,
      changeAmount: -0.061,
      turnoverRate: 0,
    },
    {
      date: '2026-05-18',
      open: 3.396,
      close: 3.458,
      low: 3.396,
      high: 3.551,
      volume: 1540181000,
      amount: 5353119800,
      amplitude: 4.53,
      changePercent: 1.05,
      changeAmount: 0.036,
      turnoverRate: 0,
    },
    {
      date: '2026-05-19',
      open: 3.411,
      close: 3.594,
      low: 3.338,
      high: 3.599,
      volume: 1686070400,
      amount: 5826910800,
      amplitude: 7.55,
      changePercent: 3.93,
      changeAmount: 0.136,
      turnoverRate: 0,
    },
    {
      date: '2026-05-20',
      open: 3.567,
      close: 3.766,
      low: 3.559,
      high: 3.767,
      volume: 1780503900,
      amount: 6569839500,
      amplitude: 5.79,
      changePercent: 4.79,
      changeAmount: 0.172,
      turnoverRate: 0,
    },
    {
      date: '2026-05-21',
      open: 3.841,
      close: 3.569,
      low: 3.556,
      high: 3.865,
      volume: 2196395600,
      amount: 8187555100,
      amplitude: 8.2,
      changePercent: -5.23,
      changeAmount: -0.197,
      turnoverRate: 0,
    },
    {
      date: '2026-05-22',
      open: 3.594,
      close: 3.641,
      low: 3.529,
      high: 3.656,
      volume: 1516517000,
      amount: 5456216200,
      amplitude: 3.56,
      changePercent: 2.02,
      changeAmount: 0.072,
      turnoverRate: 0,
    },
    {
      date: '2026-05-25',
      open: 3.64,
      close: 3.904,
      low: 3.6,
      high: 3.91,
      volume: 1821901600,
      amount: 6898652600,
      amplitude: 8.51,
      changePercent: 7.22,
      changeAmount: 0.263,
      turnoverRate: 0,
    },
    {
      date: '2026-05-26',
      open: 3.889,
      close: 3.817,
      low: 3.713,
      high: 3.889,
      volume: 1928813500,
      amount: 7288451800,
      amplitude: 4.51,
      changePercent: -2.23,
      changeAmount: -0.087,
      turnoverRate: 0,
    },
    {
      date: '2026-05-27',
      open: 3.84,
      close: 3.707,
      low: 3.695,
      high: 3.907,
      volume: 1660319800,
      amount: 6315100800,
      amplitude: 5.55,
      changePercent: -2.88,
      changeAmount: -0.11,
      turnoverRate: 0,
    },
    {
      date: '2026-05-28',
      open: 3.661,
      close: 3.798,
      low: 3.66,
      high: 3.818,
      volume: 1633370800,
      amount: 6146167400,
      amplitude: 4.26,
      changePercent: 2.45,
      changeAmount: 0.091,
      turnoverRate: 0,
    },
    {
      date: '2026-05-29',
      open: 3.798,
      close: 3.586,
      low: 3.533,
      high: 3.822,
      volume: 1960051700,
      amount: 7154291700,
      amplitude: 7.61,
      changePercent: -5.58,
      changeAmount: -0.212,
      turnoverRate: 0,
    },
    {
      date: '2026-06-01',
      open: 3.58,
      close: 3.37,
      low: 3.364,
      high: 3.599,
      volume: 1870293300,
      amount: 6489098300,
      amplitude: 6.55,
      changePercent: -6.02,
      changeAmount: -0.216,
      turnoverRate: 0,
    },
    {
      date: '2026-06-02',
      open: 3.377,
      close: 3.491,
      low: 3.332,
      high: 3.492,
      volume: 1449107700,
      amount: 4944573900,
      amplitude: 4.75,
      changePercent: 3.59,
      changeAmount: 0.121,
      turnoverRate: 0,
    },
  ],
  financialRows: [
    {
      code: '688981',
      name: '中芯国际',
      weight: 9.8,
      productTags: ['晶圆代工', '先进制程', '成熟制程'],
      intro: '国内晶圆代工龙头，决定组合对先进制程、成熟制程和国产制造能力的核心暴露。',
      data: [
        {
          year: 2026,
          quarter: 1,
          roe: 0.9,
          mainBusinessGrowthRate: 8.07,
          netProfitMargin: 9.05,
          grossProfitMargin: 21.48,
          revenueGrowthRate: 8.07,
          nonNetProfitGrowthRate: 5.32,
        },
      ],
    },
    {
      code: '688041',
      name: '海光信息',
      weight: 9.66,
      productTags: ['国产CPU', 'DCU', 'AI算力'],
      intro: '国产高端处理器和DCU供应商，是组合中AI算力芯片国产替代弹性的重要来源。',
      data: [
        {
          year: 2026,
          quarter: 1,
          roe: 2.99,
          mainBusinessGrowthRate: 68.06,
          netProfitMargin: 21.75,
          grossProfitMargin: 55.6,
          revenueGrowthRate: 68.06,
          nonNetProfitGrowthRate: 34.99,
        },
      ],
    },
    {
      code: '688256',
      name: '寒武纪',
      weight: 9.56,
      productTags: ['AI芯片', '智能加速卡', '云边端推理'],
      intro: 'AI芯片和智能计算加速卡龙头，业绩高增强化组合的算力芯片进攻属性。',
      data: [
        {
          year: 2026,
          quarter: 1,
          roe: 8.2,
          mainBusinessGrowthRate: 159.56,
          netProfitMargin: 35.12,
          grossProfitMargin: 54.33,
          revenueGrowthRate: 159.56,
          nonNetProfitGrowthRate: 238.56,
        },
      ],
    },
    {
      code: '688008',
      name: '澜起科技',
      weight: 7.8,
      productTags: ['内存接口芯片', 'PCIe/CXL', '服务器平台'],
      intro: '内存接口与服务器平台芯片龙头，受益AI服务器内存带宽和互连升级。',
      data: [
        {
          year: 2026,
          quarter: 1,
          roe: 5.39,
          mainBusinessGrowthRate: 19.51,
          netProfitMargin: 56.8,
          grossProfitMargin: 69.79,
          revenueGrowthRate: 19.51,
          nonNetProfitGrowthRate: 20.14,
        },
      ],
    },
    {
      code: '688012',
      name: '中微公司',
      weight: 7.3,
      productTags: ['刻蚀设备', '薄膜设备', '半导体设备'],
      intro: '半导体刻蚀设备龙头，提供国产晶圆制造设备替代的关键弹性。',
      data: [
        {
          year: 2026,
          quarter: 1,
          roe: 4,
          mainBusinessGrowthRate: 34.13,
          netProfitMargin: 31.51,
          grossProfitMargin: 39.89,
          revenueGrowthRate: 34.13,
          nonNetProfitGrowthRate: 60.09,
        },
      ],
    },
    {
      code: '688521',
      name: '芯原股份',
      weight: 4.05,
      productTags: ['芯片设计服务', 'IP授权', 'Chiplet'],
      intro: '芯片IP授权和设计服务公司，代表定制芯片、Chiplet和半导体设计服务暴露。',
      data: [
        {
          year: 2026,
          quarter: 1,
          roe: -10.38,
          mainBusinessGrowthRate: 114.47,
          netProfitMargin: -40.78,
          grossProfitMargin: 32.29,
          revenueGrowthRate: 114.47,
          nonNetProfitGrowthRate: -43.71,
        },
      ],
    },
    {
      code: '688525',
      name: '佰维存储',
      weight: 3.77,
      productTags: ['存储模组', '先进封测', 'AI存储'],
      intro: '存储模组和先进封测相关公司，受益存储周期修复和AI终端存储升级。',
      data: [
        {
          year: 2026,
          quarter: 1,
          roe: 41.98,
          mainBusinessGrowthRate: 341.53,
          netProfitMargin: 42.22,
          grossProfitMargin: 53.3,
          revenueGrowthRate: 341.53,
          nonNetProfitGrowthRate: 1405.58,
        },
      ],
    },
    {
      code: '688072',
      name: '拓荆科技',
      weight: 3.39,
      productTags: ['薄膜沉积设备', 'PECVD', '半导体设备'],
      intro: '薄膜沉积设备供应商，跟随晶圆厂资本开支和国产设备验证节奏。',
      data: [
        {
          year: 2026,
          quarter: 1,
          roe: 8.25,
          mainBusinessGrowthRate: 56.97,
          netProfitMargin: 50.54,
          grossProfitMargin: 41.69,
          revenueGrowthRate: 56.97,
          nonNetProfitGrowthRate: 156.65,
        },
      ],
    },
    {
      code: '688498',
      name: '源杰科技',
      weight: 3.29,
      productTags: ['光芯片', '激光器芯片', '高速光通信'],
      intro: '高速光芯片和激光器芯片公司，为组合提供光通信与AI数据中心互联弹性。',
      data: [
        {
          year: 2026,
          quarter: 1,
          roe: 7.38,
          mainBusinessGrowthRate: 320.94,
          netProfitMargin: 50.51,
          grossProfitMargin: 77.81,
          revenueGrowthRate: 320.94,
          nonNetProfitGrowthRate: 1173.64,
        },
      ],
    },
    {
      code: '688347',
      name: '华虹公司',
      weight: 2.35,
      productTags: ['特色工艺', '晶圆代工', '功率半导体'],
      intro: '特色工艺晶圆代工平台，补充功率、模拟和嵌入式非易失存储等成熟制程暴露。',
      data: [
        {
          year: 2026,
          quarter: 1,
          roe: 0.31,
          mainBusinessGrowthRate: 18.22,
          netProfitMargin: -2.75,
          grossProfitMargin: 17.63,
          revenueGrowthRate: 18.22,
          nonNetProfitGrowthRate: 724.01,
        },
      ],
    },
  ],
  viewpoints: [],
  kLineMarkers: {
    candleMarkers: [{ date: '2026-05-29', label: '锤子线', position: 'below' }],
    supportMarkers: [{ date: '2026-05-22', price: 3.533, label: '支撑位', position: 'below' }],
    resistanceMarkers: [{ date: '2026-05-19', price: 3.822, label: '压力位', position: 'above' }],
    keyInfoMarkers: [{ date: '2026-05-28', label: '关键信息位', position: 'above', lineLength: 1.5 }],
    polyLines: [
      {
        points: [
          { date: '2026-05-25', price: 3.91 },
          { date: '2026-05-28', price: 3.818 },
          { date: '2026-06-02', price: 3.492 },
        ],
        lineWidth: 0.8,
        lineType: 'dashed',
        color: '#666666',
      },
    ],
  },
};

export default etf588200;
