...
- 请求参数（Query）：
  - `themeKeys` `array[string]?`，主题 key 列表，参考 `frontend/src/data/etfBaseList.js` 的 `key`，且对应配置必须有 `indexCode`
  - `days` `number?`，默认 `90`，最近交易日数量
- 返回说明：
  - `data` 为数组，每个元素包含 `date`、`sse_amount`、`szse_amount`、`total_amount`、`unit`
  - 只有传入的 `themeKeys` 才会出现在返回字段中；例如传 `themeKeys=ai`，返回里只带 `ai`
  - 若传入的 `themeKeys` 不存在或缺少 `indexCode`，接口会直接返回错误，不继续执行后续请求
